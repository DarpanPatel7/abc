@if (!empty($menu))
    {!! Form::model($menu, ['method' => 'PATCH','route' => ['designations.update', Crypt::Encrypt($menu->id)], 'id'=>'editDesignationForm']) !!}
        <div class="modal-body">
            <div class="row">
                <div class="col mb-3 inp-group">
                    <label class="form-label" for="designation_name">Designation Name</label>
                    <input type="text" class="form-control" placeholder="Designation Name" name="designation_name" value="{{ old('name', $menu->name) }}" aria-label="Designation Name" />
                </div>
            </div>
            <div class="row">
                <div class="col mb-0">
